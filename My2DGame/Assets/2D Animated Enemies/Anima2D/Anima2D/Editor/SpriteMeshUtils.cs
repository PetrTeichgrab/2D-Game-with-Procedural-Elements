	...
	public static class SpriteMeshUtils
	{
		...
		internal static void ApplyToSprite(this SpriteMeshData spriteMeshData, Sprite sprite)
        {
			...
			if (spriteBones.Length > 0)
			{
				...
				var weightedTriangles = new List<WeightedTriangle>(indices.Length / 3);
				...
				var sortedIndices = new List<int>(indices.Length);
						
				for(var i = 0; i < weightedTriangles.Count; ++i)
				{
					var t = weightedTriangles[i];
					sortedIndices.Add(t.p1);
					sortedIndices.Add(t.p2);
					sortedIndices.Add(t.p3);
				}

				indices = sortedIndices.ToArray();
			}
